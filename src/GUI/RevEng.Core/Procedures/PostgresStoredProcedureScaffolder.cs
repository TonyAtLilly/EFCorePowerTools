...
{
    public class PostgresStoredProcedureScaffolder : SqlServerRoutineScaffolder, IProcedureScaffolder
    {
        private const string ParameterPrefix = "parameter";

        public PostgresStoredProcedureScaffolder([NotNull] ICSharpHelper code)
            : base(code)
        {
        ...
        private static string GetDbContextExtensionsText(bool useAsyncCalls)
        {
            ...
            var assembly = typeof(PostgresStoredProcedureScaffolder).GetTypeInfo().Assembly;
        ...
        private static List<string> CreateUsings(ModuleScaffolderOptions scaffolderOptions, RoutineModel model)
        {
            ...
            if (model.Routines.SelectMany(r => r.Parameters).Any(p => p.PgClrType() == typeof(Geometry)))
            {
                usings.AddRange(new List<string>()
                {
                    "using NetTopologySuite.Geometries",
        ...
        private static string GenerateProcedureStatement(Routine procedure, string retValueName, bool useAsyncCalls)
        {
            ...
            //var fullExec = $"\"CALL @{retValueName} = [{procedure.Schema}].[{procedure.Name}] {string.Join(", ", paramList)}\", sqlParameters{(useAsyncCalls ? ", cancellationToken" : string.Empty)}".Replace(" \"", "\"", StringComparison.OrdinalIgnoreCase);
            var fullExec = $"\"SELECT \"{procedure.Name}\"({string.Join(", ", paramList)})\", sqlParameters{(useAsyncCalls ? ", cancellationToken" : string.Empty)}".Replace(" \"", "\"", StringComparison.OrdinalIgnoreCase);
        ...
        private void GenerateDapperSupport(bool useAsyncCalls)
        {
            ...
            using (Sb.Indent())
            {
                Sb.AppendLine("private static DynamicParameters CreateDynamic(NpgsqlParameter[] sqlParameters)");
                Sb.AppendLine("{");
                using (Sb.Indent())
                {
        ...
        private void GenerateProcedure(Routine procedure, RoutineModel model, bool signatureOnly, bool useAsyncCalls)
        {
            var paramStrings = procedure.Parameters.Where(p => !p.Output)
                .Select(p => $"{Code.Reference(p.PgClrType(asMethodParameter: true))} {Code.Identifier(p.Name)}")
                .ToList();
            ...
            var outParamStrings = outParams
                .Select(p => $"OutputParameter<{Code.Reference(p.PgClrType())}> {Code.Identifier(p.Name)}")
                .ToList();
        ...
        private void GenerateParameter(ModuleParameter parameter, Routine procedure)
        {
            ...
            var sqlDbType = parameter.PgDbType();
            ...
            using (Sb.Indent())
            {
                ...
                Sb.AppendLine($"DbType = System.Data.DbType.{sqlDbType},");

                //if (sqlDbType == DbType.Structured)
                //{
                //    Sb.AppendLine($"TypeName = \"{parameter.TypeName}\",");
                //}

                //if (sqlDbType == DbType.Udt)
                //{
